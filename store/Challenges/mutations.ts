...
export default {
    ...
    [Mutations.COMPLETE_CHALLENGE] (state, xpAmount) {
        const { current, end } = state.xp; //Acessa os valores atuais e finais de xp
        const currentTotalXp = current + xpAmount
        const shouldLevelUp = (currentTotalXp) >= end; //Constante para saber se precisa saber de nível
